defmodule Day2 do
  ...
  def checksum_part2(str) do
    str
    |> String.split("\n")
    |> Enum.map(&format_line/1)
    |> Enum.map(&process_line_2/1)
    |> Enum.sum
  end
  ...
  def process_line_2([]), do: 0
  def process_line_2(line) do
    (for n <- line, x <- line, n != x and n > x and rem(n, x) == 0, do: div(n, x))
    |> hd
  end
end

input = """
3925	858	2203	440	185	2886	160	1811	4272	4333	2180	174	157	361	1555
...
37	1069	166	1037	172	258	1071	90	497	1219	145	1206	143	153	1067	510
    """
Day2.checksum_part1(input) |> IO.inspect
Day2.checksum_part2(input) |> IO.inspect
